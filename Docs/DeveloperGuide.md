...
The macOS player is built with `bash Tools/unity_mac.sh build` and writes to `Unity/Builds/Albion Odyssey.app`. The reproducible packaging script is `Tools/package_mac_release.sh`; it creates an ad-hoc signed local archive and SHA-256 manifest. Developer ID signing and notarization require Apple credentials.
...
The latest results are recorded in [Docs/Verification-v0.11](Verification-v0.11). The focused walkable-building check covers Robinson, Bonta and the four-wing Science Complex. Local runtime reports are written to the ignored `Verification/` directory.
