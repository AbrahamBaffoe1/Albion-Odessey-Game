...
{
    [Serializable] public sealed class TourMedia { public string label,kind,url,caption; }
    ...
    [Serializable] public sealed class TourCatalog
    {
        ...
        public bool Valid()=>places!=null&&places.Length>0&&places.All(p=>!string.IsNullOrWhiteSpace(p.id)&&!string.IsNullOrWhiteSpace(p.summary)&&p.campusIds!=null&&p.media!=null&&SafeSource(p.source)&&p.media.All(m=>!string.IsNullOrWhiteSpace(m.caption)&&SafeMedia(m.url)&&(m.kind=="photo"||m.kind=="panorama"||m.kind=="video")))&&places.Select(p=>p.id).Distinct().Count()==places.Length&&places.SelectMany(p=>p.campusIds).Distinct().Count()==places.Sum(p=>p.campusIds.Length);
