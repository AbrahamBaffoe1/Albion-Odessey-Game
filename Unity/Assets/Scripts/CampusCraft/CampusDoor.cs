...
namespace AlbionOdyssey
{
    ...
    public sealed class CampusDoor : MonoBehaviour
    {
        ...
        public void Build(float opening,float height,Material material)
        {
            ...
            tag=gameObject.AddComponent<CampusWorldLabel>();tag.Configure("OPEN "+Label.ToUpperInvariant(),new Color(1f,.76f,.28f),new Vector3(0,height+.28f,0),8f);UpdateLabel(height);
        }
        ...
        void UpdateLabel(float height){if(tag!=null)tag.SetText((IsOpen?"CLOSE ":"OPEN ")+Label.ToUpperInvariant());}
