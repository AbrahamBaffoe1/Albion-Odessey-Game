...
namespace AlbionOdyssey
{
    ...
    public sealed class OdysseyAccessibility : MonoBehaviour
    {
        ...
        OdysseyGame game; bool open; GUIStyle title, text, button;
        ...
        {
            if (!open && Input.GetKeyDown(KeyCode.F4)) { open = true; game.player.controls = false; Cursor.lockState = CursorLockMode.None; Cursor.visible = true; return true; }
        ...
        void OnGUI()
        {
            ...
            GUI.color = HighContrast ? Color.black : new Color(.018f, .03f, .05f, .98f); GUI.DrawTexture(new Rect(0, 0, w, h), Texture2D.whiteTexture); GUI.color = Color.white;
